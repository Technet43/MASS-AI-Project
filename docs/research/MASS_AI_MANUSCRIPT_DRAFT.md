...
> **Research draft.** The experimental results in this document are based on synthetic data unless a separate, documented dataset is cited. They are not field-validation or operational-impact evidence. See the [real-data requirements](../product/REAL_DATA_REQUIREMENTS.md) for the project’s validation boundaries.
